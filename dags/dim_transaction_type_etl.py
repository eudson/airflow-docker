...
def dim_transaction_type_etl_dag():
    @task()
    def oracle_to_clickhouse(**context):
        ...
        query = f"SELECT ID, CODE, DESCRIPTION, FISCAL_YEAR_ID, APPLICATION_ID, CREATION_DATE FROM MEX.TRANSACTION_TYPE WHERE {where_clause} ORDER BY ID ASC"
        ...
        if not result.empty:
            ...
            clickhouse_client.insert('dim_transaction_type', result.to_records(index=False),
                                     column_names=['source_id', 'code', 'description', 'fiscal_year_id',
                                                   'application_id'])
    ...
    audit_task = SourceDataAuditOperator(task_id='dim_transaction_type_etl_audit_task',
                                         source_dag_id='dim_transaction_type_etl_dag')

    oracle_to_clickhouse() >> audit_task


dim_transaction_type_etl_dag()
